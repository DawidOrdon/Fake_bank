<?php
    ...
    if(@$result['user_id']==$_SESSION['user_id'])// sprawdzanie właściciela konta
    {
        ...
        $sql_save=$db->prepare("SELECT value+interest as 'money' FROM `investment` WHERE iban=?");//pobranie wartosci zaoszczedzonyc pieniedzy
        ...
        $sql=$db->prepare("UPDATE `investment` SET `value` = '0', `interest` = '0' WHERE `iban` = ?");//wyzerowanie konta oszczednosciowego
        $sql->bind_param('s',$_GET['iban']);
        $sql->execute();
        echo $save_money;
